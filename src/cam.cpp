...
static gchar* target = (gchar*)"rtsp";
...
static gboolean noattach = FALSE;
static gboolean roiOff = FALSE;
static gboolean fileloop = FALSE;
static GOptionEntry entries[] =
{
    ...
    { "camid", 'c', 0, G_OPTION_ARG_INT, &camid, "cam ID", "0"},
    { "fileloop", 'l', 0, G_OPTION_ARG_NONE, &fileloop, "loop file source ", "loop file source"},
    ...
    { "noattach", 'a', 0, G_OPTION_ARG_NONE, &noattach, "Not attach AI info, default: true", NULL },
...
main (int argc, char *argv[])
{
...

    {
        ...
        if (!nodet) {
            sprintf(pip + strlen(pip), " \
                ...
                ! queue ! vvas_xfilter name=refinedet kernels-config=\"%s/refinedet.json\" \
                ! queue ! vvas_xfilter name=crop      kernels-config=\"%s/crop.json\" \
                ! queue ! scalem.sink_master vvas_xmetaaffixer name=scalem scalem.src_master \
                ! fakesink \
                t.src_1 \
    ...
    if (!noattach) {
        sprintf(pip + strlen(pip), " \
            ! queue ! aibox_dist_attach roi-max-num=%d \
            "
            , camid
            );
    ...
    if (std::string(target) == "rtsp")
    {
        ...
        sprintf(pip + strlen(pip), " \
                ! h264parse ! queue ! mpegtsmux ! queue ! rtpmp2tpay name=pay0 pt=33 )");
